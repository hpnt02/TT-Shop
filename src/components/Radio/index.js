...
function RadioItem ({data, ...props}) {
  ...
  return (
    ...
    <Radio.Group onChange={onChange} value={value} style={{margin:'10px 0'}} defaultValue="All">
        <Radio value="All" style={{lineHeight:'40px', paddingLeft:'10px', width:'100%'}}>Tất cả</Radio>
      {  data.map((value, index) => {
            const values = value.nameNCC || value.nameLoaiSP
            return (
                <Radio key={index} value={value._id} style={{lineHeight:'40px', paddingLeft:'10px', width:'100%'}}>{values}</Radio>
            )
        })}
    </Radio.Group>
    }
